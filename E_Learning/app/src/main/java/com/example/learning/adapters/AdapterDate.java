...
public class AdapterDate extends BaseAdapter {
    ...
    public AdapterDate(Context context, List<Integer> list) {
    ...
    public View getView(final int i, View view, ViewGroup viewGroup) {
        ...
        if (view == null) {
            view = LayoutInflater.from(context).inflate(R.layout.item_week_gv, null);
...
//                    Toast.makeText(context, "Already sign in!", Toast.LENGTH_SHORT).show();
                    if (onCalendarItemClick != null) {
                        onCalendarItemClick.onClick(i - 2);
                    }
                } else {

                    Toast.makeText(context, "本日无数据", Toast.LENGTH_SHORT).show();
//                    status.set(i, true);
...
//                    }
    ...
    public void setOnItemClick(OnCalendarItemClick onItemClick) {
        this.onCalendarItemClick = onItemClick;
    }
}
